...
export async function fetchIndividualPublicationDetail(
  ...
  return cluster.execute(publicationLink, async ({ page }) => {
    try {
      ...
      console.log(
        `Error fetching details for ${publicationLink}:`,
        error.message
      );
      return null;
    }
...
async function fetchAllPublicationDetails(authorId) {
  ...
  console.log(`Fetched ${allPublicationDetails.length} publications`);
...
(async () => {
  const authorId = "kAS_U9YAAAAJ";
  const publications = await fetchAllPublicationDetails(authorId);
